...
from projects.mmdet3d_plugin.GenAD.generator import LaneNet, MLP
...
class GenADHead(DETRHead):
    ...
    def forward(self,
        ...
        """GenADHead forward function.
        Args:
            mlvl_feats: Features from the upstream network, each is a 5D-tensor with shape
            prev_bev: previous bev featues
            only_bev: only compute BEV features with encoder.
            ego_his_trajs: ego history trajectory
            gt_labels_3d: 3d gt boxes for objects in future
            gt_attr_labels: attribution label for objects in future
            ego_fut_trajs:  ego future trajectories

        Returns:
             bev_embed: bev_embed features
             all_cls_scores: object predicted classes
             all_bbox_preds: object predicted coordinates
             all_traj_preds : agent predicted trajectories
             all_traj_cls_scores : agent predicted classes
             map_all_cls_scores : map predicted classes
             map_all_bbox_preds : map predicted coordinates
             map_all_pts_preds : map predicted pts_coordinates
             ego_fut_preds : ego predticed trajectories
             loss_vae_gen : distribution params for compute kl loss
        """

        batch_size, num_cam, _, _, _ = mlvl_feats[0].shape

        if only_bev:
            outputs = self.prepare_features(mlvl_feats, img_metas, prev_bev, only_bev)
            return outputs
        else:
            outputs = self.prepare_features(mlvl_feats, img_metas, prev_bev, only_bev)
        ...
        det_outputs = self.det_predict_heads(
            hs=hs,
            init_reference=init_reference,
            inter_references=inter_references)

        outputs_classes, outputs_coords, outputs_coords_bev = det_outputs

        map_outputs = self.map_predict_heads(
            bs=batch_size,
            map_hs=map_hs,
            map_init_reference=map_init_reference,
            map_inter_references=map_inter_references)

        map_outputs_classes, map_outputs_coords, map_outputs_pts_coords, map_outputs_coords_bev = map_outputs

        outputs_trajs_classes = []

        motion_hs, ca_motion_query = self.environmental_interaction(
            hs=hs,
            map_hs=map_hs,
            ego_his_trajs=ego_his_trajs,
            outputs_coords_bev=outputs_coords_bev,
            outputs_classes=outputs_classes,
            map_outputs_classes=map_outputs_classes,
            map_outputs_coords_bev=map_outputs_coords_bev)

        # establish a distribution model for trajectory prediction
        motion_hs = motion_hs.permute(1, 0, 2)
        ca_motion_query = ca_motion_query.reshape(batch_size, -1, self.embed_dims)

        current_states = torch.cat((motion_hs, ca_motion_query), dim=2)
        ...
        if self.training:
            future_distribution_inputs = self.get_future_labels(
                ...
                device=motion_hs.device)
        else:
            future_distribution_inputs = None

        # model distribution for state
        if self.fut_ts > 0:
             if self.probabilistic:
                 sample, output_distribution = self.distribution_forward(
                     present_features=current_states,
                     future_distribution_inputs=future_distribution_inputs,
                     noise=noise)
                 distribution_comp = {**distribution_comp, **output_distribution}

        # predict future distribution from distribution
        ...
        # autoregressive predict future trajectories for ego
        for i in range(self.fut_ts):
                outputs_ego_trajs = self.ego_fut_decoder(ego_query_hs[i])
                outputs_ego_trajs = outputs_ego_trajs.reshape(batch_size, self.ego_fut_mode, 2)
        ...
        # predict heads for different tasks
        ...
        outputs_trajs = torch.repeat_interleave(outputs_trajs, outputs_coords.shape[0], dim=0)
        outputs_trajs_classes = torch.repeat_interleave(outputs_trajs_classes, outputs_coords.shape[0], dim=0)
        ...
        outs = {
            ...
            'all_traj_cls_scores': outputs_trajs_classes,
    ...
    def prepare_features(self,
        ...
        """ decode various feature from different decoder.
        Args:
            mlvl_feats: Features from the upstream network, each is a 5D-tensor with shape
            prev_bev: previous bev featues
            only_bev: only compute BEV features with encoder.
        Returns:
            outputs: only bev features or various feature
        """
        ...
        if only_bev:  # only use encoder to obtain BEV features, TODO: refine the workaround
            outputs = self.transformer.get_bev_features(
                mlvl_feats,
                bev_queries,
                self.bev_h,
                self.bev_w,
    ...
    def det_predict_heads(self, hs, init_reference, inter_references):
        """ object detection branches.
        Args:
            hs: det branches decoder features
            init_reference: initial references
            inter_references: intermediate references.
        Returns:
            outputs_classes: objects classes
            outputs_coords: objects coordinates
            outputs_coords_bev: objects bev coordinates
        """
        outputs_classes = []
        outputs_coords = []
        outputs_coords_bev = []

        # decode agents coords and class by cross attention with bev features
        ...
        return outputs_classes, outputs_coords, outputs_coords_bev

    def map_predict_heads(self, bs, map_hs, map_init_reference, map_inter_references):
        """ map predict branches.
        Args:
            bs: batch size
            map_hs: maps branches decoder features
            map_init_reference: initial references
            map_inter_references: intermediate references.
        Returns:
            map_outputs_classes: maps classes
            map_outputs_coords: maps coordinates
            map_outputs_coords_bev: maps bev coordinates
        """
        map_outputs_classes = []
        map_outputs_coords = []
        ...
        return map_outputs_classes, map_outputs_coords, map_outputs_pts_coords, map_outputs_coords_bev


    def environmental_interaction(self, hs, map_hs, ego_his_trajs, outputs_coords_bev, outputs_classes, map_outputs_classes, map_outputs_coords_bev):
        """ ego interaction with agents and maps.
        Args:
            hs: det branches decoder features
            map_hs: maps branches decoder features
            ego_his_trajs: ego history trajectory
            ...
            map_outputs_coords_bev: maps bev coordinates
        Returns:
            motion_hs: features for agents trajectories prediction
            ca_motion_query: features for ego planning prediction
        """
        if self.motion_decoder is not None:
            ...
            # agents and ego interaction with self-attention
            ...
            # agents, ego interact with map with cross-attention
            if self.motion_map_decoder is not None:
                # map preprocess
                motion_coords = outputs_coords_bev[-1]  # [B, A, 2]
        ...
        return motion_hs, ca_motion_query
    ...
    def future_states_predict(self, batch_size, sample, hidden_states, current_states):
        ...
        current_states_hs = current_states.unsqueeze(0).repeat(self.fut_ts, 1, 1, 1)
